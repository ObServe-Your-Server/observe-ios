...
class LiveNetworkInFetcher: ObservableObject {
    @Published var entries: [NetworkInEntry] = []
    ...
    struct NetworkInEntry: Identifiable {
        let id = UUID()
    ...
    private func fetch() {
        ...
        let end = now
        let endpoint = "/metrics/network/in"
        let queryItems = [
            URLQueryItem(name: "startTime", value: "\(start)"),
            URLQueryItem(name: "endTime", value: "\(end)"),
            URLQueryItem(name: "interval", value: "5")
        ]
        networkService.fetch(endpoint: endpoint, queryItems: queryItems) { [weak self] (result: Result<NetworkInResponse, Error>) in
                ...
                case .success(let response):
                    let allEntries = response.data.result.flatMap { result in
                        result.values.map {
                            ...
                            NetworkInEntry(timestamp: $0.timestamp, value: $0.value / 1024)
                        }
                    }
